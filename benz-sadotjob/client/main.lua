...
local function MenuGarage()
    ...
    for k in pairs(Config.Vehicles) do
        towMenu[#towMenu+1] = {
            ...
            params = {
                event = "benz-sadotjob:client:TakeOutVehicle",
                args = {
                    vehicle = k
                }
            }
...
local function CreateZone(type, number)
    ...
    if type == "main" then
        event = "benz-sadotjob:client:PaySlip"
    ...
    elseif type == "vehicle" then
        event = "benz-sadotjob:client:Vehicle"
    ...
    elseif type == "towspots" then
        event = "benz-sadotjob:client:SpawnNPCVehicle"
    ...
    if Config.UseTarget and type == "main" then
        ...
        zoneCombo:onPlayerInOut(function(isPointInside)
            if isPointInside then
                if type == "main" then
                    TriggerEvent('benz-sadotjob:client:PaySlip')
                elseif type == "vehicle" then
                    TriggerEvent('benz-sadotjob:client:Vehicle')
                elseif type == "towspots" then
                    TriggerEvent('benz-sadotjob:client:SpawnNPCVehicle')
                end
            end
        end)
        if type == "vehicle" then
            ...
            zoneComboV:onPlayerInOut(function(isPointInside)
                if isPointInside then
                    TriggerEvent('benz-sadotjob:client:ShowMarker', true)
                else
                    TriggerEvent('benz-sadotjob:client:ShowMarker', false)
                end
            end)
        elseif type == "towspots" then
...
RegisterNetEvent('benz-sadotjob:client:SpawnVehicle', function()
...
RegisterNetEvent('jobs:client:ToggleNpc', function()
    if QBCore.Functions.GetPlayerData().job.name == "sadot" then
...
RegisterNetEvent('benz-sadotjob:client:TowVehicle', function()
                    ...
                    if #(towPos - targetPos) < 11.0 then
                        QBCore.Functions.Progressbar("towing_vehicle", Lang:t("mission.towing_vehicle"), 5000, false, true, {
                            ...
                            if NpcOn then
                                ...
                                TriggerServerEvent('benz-sadotjob:server:nano', vehNetID)
                                --remove zone
...
RegisterNetEvent('benz-sadotjob:client:TakeOutVehicle', function(data)
    local coords = Config.Locations["vehicle"].coords
    ...
    if #(pos - coords) <= 5 then
        ...
        TriggerServerEvent('benz-sadotjob:server:DoBail', true, vehicleInfo)
...
RegisterNetEvent('benz-sadotjob:client:Vehicle', function()
    local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
    if not CurrentTow then
        if vehicle and isTowVehicle(vehicle) then
            DeleteVehicle(GetVehiclePedIsIn(PlayerPedId()))
            TriggerServerEvent('benz-sadotjob:server:DoBail', false)
        else
            MenuGarage()
        end
    else
...
RegisterNetEvent('benz-sadotjob:client:PaySlip', function()
    if JobsDone > 0 then
        RemoveBlip(CurrentBlip)
        TriggerServerEvent("benz-sadotjob:server:11101110", JobsDone)
        JobsDone = 0
        NpcOn = false
    else
...
RegisterNetEvent('benz-sadotjob:client:SpawnNPCVehicle', function()
...
RegisterNetEvent('benz-sadotjob:client:ShowMarker', function(active)
    if PlayerJob.name == "sadot" then
        showMarker = active
    end
end)
