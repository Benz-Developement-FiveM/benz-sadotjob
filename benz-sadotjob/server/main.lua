...
RegisterNetEvent('benz-sadotjob:server:DoBail', function(bool, vehInfo)
    local src = source
    ...
    if bool then
        if Player.PlayerData.money.cash >= Config.BailPrice then
            ...
            TriggerClientEvent('benz-sadotjob:client:SpawnVehicle', src, vehInfo)
...
RegisterNetEvent('benz-sadotjob:server:nano', function(vehNetID)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    if Player.PlayerData.job.name ~= 'sadot' or dist > 11.0 then
        return DropPlayer(src, Lang:t('info.skick'))
    end
    local chance = math.random(1, 100)
    if chance < 26 then
--        exports['qb-inventory']:AddItem(src, '', 1, false, false, 'benz-sadotjob:server:nano')
--        TriggerClientEvent('qb-inventory:client:ItemBox', src, QBCore.Shared.Items[''], 'add')
    end
end)

RegisterNetEvent('benz-sadotjob:server:11101110', function(drops)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
    local playerPed = GetPlayerPed(src)
    local playerCoords = GetEntityCoords(playerPed)
    if Player.PlayerData.job.name ~= 'sadot' or #(playerCoords - vector3(Config.Locations['main'].coords.x, Config.Locations['main'].coords.y, Config.Locations['main'].coords.z)) > 6.0 then
        return DropPlayer(src, Lang:t('info.skick'))
    end
...
QBCore.Commands.Add('dottow', Lang:t('info.tow'), {}, false, function(source)
    local Player = QBCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == 'sadot' then
        TriggerClientEvent('benz-sadotjob:client:TowVehicle', source)
    end
end)
